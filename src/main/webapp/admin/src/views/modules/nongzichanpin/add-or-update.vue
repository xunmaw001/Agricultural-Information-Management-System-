<template>
	<div class="addEdit-block" :style='{"padding":"30px 0 0 0"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="产品名称" prop="chanpinmingcheng">
					<el-input v-model="ruleForm.chanpinmingcheng" placeholder="产品名称" clearable  :readonly="ro.chanpinmingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="产品名称" prop="chanpinmingcheng">
					<el-input v-model="ruleForm.chanpinmingcheng" placeholder="产品名称" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="upload" v-if="type!='info' && !ro.chanpintupian" label="产品图片" prop="chanpintupian">
					<file-upload
						tip="点击上传产品图片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="upload" v-else-if="ruleForm.chanpintupian" label="产品图片" prop="chanpintupian">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="select" v-if="type!='info'"  label="农资类型" prop="nongzileixing">
					<el-select :disabled="ro.nongzileixing" v-model="ruleForm.nongzileixing" placeholder="请选择农资类型" >
						<el-option
							v-for="(item,index) in nongzileixingOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="农资类型" prop="nongzileixing">
					<el-input v-model="ruleForm.nongzileixing"
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="textarea" v-if="type!='info'" label="产品特点" prop="chanpintedian">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="产品特点"
					  v-model="ruleForm.chanpintedian" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else-if="ruleForm.chanpintedian" label="产品特点" prop="chanpintedian">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.chanpintedian}}</span>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-if="type!='info'"  label="产品详情" prop="chanpinxiangqing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else-if="ruleForm.chanpinxiangqing" label="产品详情" prop="chanpinxiangqing">
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='nongzileixing'){
							this.ruleForm.nongzileixing = obj[o];
							this.ro.nongzileixing = true;
							continue;
						}
						if(o=='chanpintedian'){
							this.ruleForm.chanpintedian = obj[o];
							this.ro.chanpintedian = true;
							continue;
						}
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.chanpinxiangqing = this.ruleForm.chanpinxiangqing.replace(reg,'../../../ssm9927h/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `nongzichanpin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.nongzichanpinCrossAddOrUpdateFlag = false;
						 this.parent.search();
