<template>
	<div class="addEdit-block" :style='{"padding":"30px 0 0 0"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="农产品类型" prop="nongchanpinleixing">
					<el-input v-model="ruleForm.nongchanpinleixing" placeholder="农产品类型" clearable  :readonly="ro.nongchanpinleixing"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="农产品类型" prop="nongchanpinleixing">
					<el-input v-model="ruleForm.nongchanpinleixing" placeholder="农产品类型" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
    ...
    info(id) {
      this.$http({
        url: `nongchanpinleixing/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `nongchanpinleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.nongchanpinleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
